...
{
	public DBIconRecord[] recordArray = new DBIconRecord[]{};
	private static Dictionary<int, DBIconRecord> recordDict = null;
	...
	public void Init()
	{
		...
		if(recordDict != null)
			return;
		recordDict = new Dictionary<int, DBIconRecord>();
		for(int i=0; i<recordArray.Length; i++)
		{
			DBIconRecord record = recordArray[i];
	...
	//获取记录，如果不存在返回null
	public static DBIconRecord GetRecord(int iconID, bool errorMsg = true)
	{
		if(instance == null){
			Debug.LogError("表DBIconTable未加载");
			return null;
		}
		DBIconRecord record = null;
