...
{
	public DBActorAttributeRecord[] recordArray = new DBActorAttributeRecord[]{};
	private static Dictionary<int, DBActorAttributeRecord> recordDict = null;
	...
	public void Init()
	{
		...
		if(recordDict != null)
			return;
		recordDict = new Dictionary<int, DBActorAttributeRecord>();
		for(int i=0; i<recordArray.Length; i++)
		{
			DBActorAttributeRecord record = recordArray[i];
	...
	//获取记录，如果不存在返回null
	public static DBActorAttributeRecord GetRecord(int actorID, bool errorMsg = true)
	{
		if(instance == null){
			Debug.LogError("表DBActorAttributeTable未加载");
			return null;
		}
		DBActorAttributeRecord record = null;
