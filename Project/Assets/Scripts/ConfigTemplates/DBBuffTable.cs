...
{
	public DBBuffRecord[] recordArray = new DBBuffRecord[]{};
	private static Dictionary<int, DBBuffRecord> recordDict = null;
	...
	public void Init()
	{
		...
		if(recordDict != null)
			return;
		recordDict = new Dictionary<int, DBBuffRecord>();
		for(int i=0; i<recordArray.Length; i++)
		{
			DBBuffRecord record = recordArray[i];
	...
	//获取记录，如果不存在返回null
	public static DBBuffRecord GetRecord(int ID, bool errorMsg = true)
	{
		if(instance == null){
			Debug.LogError("表DBBuffTable未加载");
			return null;
		}
		DBBuffRecord record = null;
