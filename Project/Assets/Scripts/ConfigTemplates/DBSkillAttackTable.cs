...
{
	public DBSkillAttackRecord[] recordArray = new DBSkillAttackRecord[]{};
	private static Dictionary<int, DBSkillAttackRecord> recordDict = null;
	...
	public void Init()
	{
		...
		if(recordDict != null)
			return;
		recordDict = new Dictionary<int, DBSkillAttackRecord>();
		for(int i=0; i<recordArray.Length; i++)
		{
			DBSkillAttackRecord record = recordArray[i];
	...
	//获取记录，如果不存在返回null
	public static DBSkillAttackRecord GetRecord(int ID, bool errorMsg = true)
	{
		if(instance == null){
			Debug.LogError("表DBSkillAttackTable未加载");
			return null;
		}
		DBSkillAttackRecord record = null;
