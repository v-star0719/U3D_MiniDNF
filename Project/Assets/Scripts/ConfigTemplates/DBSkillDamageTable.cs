...
{
	public DBSkillDamageRecord[] recordArray = new DBSkillDamageRecord[]{};
	private static Dictionary<int, DBSkillDamageRecord> recordDict = null;
	...
	public void Init()
	{
		...
		if(recordDict != null)
			return;
		recordDict = new Dictionary<int, DBSkillDamageRecord>();
		for(int i=0; i<recordArray.Length; i++)
		{
			DBSkillDamageRecord record = recordArray[i];
	...
	//获取记录，如果不存在返回null
	public static DBSkillDamageRecord GetRecord(int ID, bool errorMsg = true)
	{
		if(instance == null){
			Debug.LogError("表DBSkillDamageTable未加载");
			return null;
		}
		DBSkillDamageRecord record = null;
